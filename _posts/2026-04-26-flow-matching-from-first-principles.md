...
date: 2026-04-26
permalink: /posts/2026/04/sample-space-perspective-on-flow-matching/
tags:
  - flow matching
  - generative models
  - optimal transport
...
Despite being formally correct, I often found the standard presentation hard to follow. One usually starts with a path of probability distributions, introduces a velocity field through a continuity equation, and only later arrives at a tractable training objective. Along the way, auxiliary variables may appear as an additional device that makes the objective tractable.

From the sample-space point of view, these auxiliary variables are not a separate trick. They are simply part of how we choose to construct the random path. What matters is not whether the path was built from endpoints alone or from endpoints together with extra randomness. What matters is that we can sample $X_t$ and compute its pathwise derivative $\dot X_t$. Once we can do that, the velocity field is the conditional average of this derivative.

In this post, I want to show how the usual flow matching machinery can be derived naturally from a sample-path perspective, where the construction is more direct.
...
<figure style="border: 1px solid #d9dee7; border-radius: 6px; padding: 0.85rem; margin: 1.75rem 0; background: #fbfcfd;">
  <img src="/images/flow-matching/02-conditional-average-velocity.png" alt="Conditional average velocity" style="display: block; width: 100%; height: auto; margin: 0 auto;">
  <figcaption style="font-size: 0.9em; line-height: 1.45; color: #555; margin-top: 0.75rem;">
    <strong>Figure 1. Conditional averaging.</strong> The faint gray cloud shows samples from the intermediate distribution at a fixed time. Near the marked point $x$, several sampled paths pass through nearby locations with different pathwise velocities. The gray arrows show these local pathwise velocities, while the purple arrow shows their local average, illustrating the conditional expectation that defines the flow-matching velocity field.
  </figcaption>
</figure>
...
<figure style="border: 1px solid #d9dee7; border-radius: 6px; padding: 0.85rem; margin: 1.75rem 0; background: #fbfcfd;">
  <img src="/images/flow-matching/03-linear-path-snapshots.png" alt="Linear interpolation between source and target samples" style="display: block; width: 100%; height: auto; margin: 0 auto;">
  <figcaption style="font-size: 0.9em; line-height: 1.45; color: #555; margin-top: 0.75rem;">
    <strong>Figure 2. Linear interpolation path.</strong> This toy example uses a standard Gaussian source distribution and a three-component mixture as the target distribution. Each panel is generated by sampling independent pairs $(X_0,X_1)$ and plotting the interpolated samples $X_t=(1-t)X_0+tX_1$ at the displayed time. The changing point cloud visualizes the induced path of laws $(\mu_t)_{t\in[0,1]}$.
  </figcaption>
</figure>
...
<figure style="border: 1px solid #d9dee7; border-radius: 6px; padding: 0.85rem; margin: 1.75rem 0; background: #fbfcfd;">
  <img src="/images/flow-matching/05-estimated-velocity-field.png" alt="Estimated velocity field for the linear path" style="display: block; width: 100%; height: auto; margin: 0 auto;">
  <figcaption style="font-size: 0.9em; line-height: 1.45; color: #555; margin-top: 0.75rem;">
    <strong>Figure 3. Estimated velocity field.</strong> The blue, green, and orange background clouds show samples from $\mu_0$, $\mu_t$, and $\mu_1$ for the same linear path, with $t=0.5$. The arrows estimate $v_t(x)$ by drawing many pairs $(X_0,X_1)$, computing their intermediate locations $X_t$ and velocities $X_1-X_0$, and then locally averaging those velocities with a Gaussian kernel. Arrows are only shown where enough intermediate samples are nearby, since the conditional-mean estimate is unstable in low-density regions.
  </figcaption>
</figure>
